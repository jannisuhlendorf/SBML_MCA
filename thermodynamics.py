...
class thermodynamics:
    ...
    def find_steady_state_flux( self, flux, ss, eq):
        """ find a steady state flux which is consisnten with the steady state concentrations and equilibrium constants"""
        import scipy.optimize
        indices = [ i for i,s in enumerate(misc.get_species_wo_enzymes(self._model)) if not(s.getConstant() or s.getBoundaryCondition()) ]
...
if __name__=='__main__':
    ...
    keq[0]=2
    keq[1]=.501
    #t.check_equilibrium_constants( keq )
